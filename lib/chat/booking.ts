 ...
 *
 * The three branches are §5.1's, and the unverified one is the branch that
 * ships today: **no timing claim at all**. This function is the only place in
 * the assistant that is allowed to say anything about callback timing, and it
 * reads BUSINESS_HOURS and nothing else — in particular not SAME_DAY_CALLBACK,
 * whose "today" promise is the homepage's and is being re-examined.
 *
 * `hours` is a parameter so the verified branches can be exercised in a test
 * without flipping a Verifiable in the repo.
 */
export function callbackExpectation(
  now: Date = new Date(),
  hours: Verifiable<BusinessHours> = BUSINESS_HOURS
): string {
  if (!hours.verified) {
    return "Someone from the team will call you back.";
  }

  const { timeZone, week } = hours.value;
  const { day, minutes } = localNow(now, timeZone);

  const today = week[day];
  if (today && minutes >= minutesOf(today.opens) && minutes < minutesOf(today.closes)) {
    return "Someone will call you today.";
  }

  // Closed. Find the next opening — later today counts.
  if (today && minutes < minutesOf(today.opens)) {
    return "The team will call you first thing this morning.";
  }
  for (let ahead = 1; ahead <= 7; ahead += 1) {
    const next = week[(day + ahead) % 7];
    if (!next) continue;
    const when = ahead === 1 ? "tomorrow" : WEEKDAY_NAME[(day + ahead) % 7];
    return `The team will call you first thing ${when}.`;
  }

  // Hours confirmed as never open. Nonsense, but not worth a wrong promise.
  return "Someone from the team will call you back.";
}
